...
func (r *threadRepository) GetThreads(ctx context.Context, currentUserID string, category string, search string, limit, offset int) ([]*domain.ThreadResponse, error) {
	if limit <= 0 {
		limit = 10
	}
	if offset < 0 {
	...
	query := `
		...
		WHERE ($2 = '' OR ($2 != 'subscribed' AND t.category = $2) OR ($2 = 'subscribed' AND t.user_id IN (SELECT target_user_id FROM user_subscriptions WHERE subscriber_id = CASE WHEN $1 = '' THEN NULL ELSE $1::uuid END)))
		  AND ($3 = '' OR t.content ILIKE '%' || $3 || '%' OR t.category ILIKE '%' || $3 || '%')
		ORDER BY t.created_at DESC
		LIMIT $4 OFFSET $5
	`
	rows, err := r.db.Query(ctx, query, currentUserID, category, search, limit, offset)
	if err != nil {
