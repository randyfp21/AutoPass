...
import { Heart, MessageSquare, Bookmark, Trash2, Car, Share2, Check } from 'lucide-react';
...
export function ThreadCard({
  ...
  const handleDelete = async () => {
    if (!window.confirm('Apakah Anda yakin ingin menghapus post thread ini?')) return;
    setIsDeleting(true);
    try {
      ...
      <div className="flex items-center justify-between pt-2 border-t border-slate-100 text-xs font-semibold">
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            onClick={handleLike}
            className={[
              'flex items-center gap-1.5 py-1 px-2.5 rounded-xl transition-all',
              thread.is_liked
          ...
          <button
            ...
            className="flex items-center gap-1.5 py-1 px-2.5 text-slate-500 hover:bg-slate-100 hover:text-slate-900 rounded-xl transition-all"
          >
          ...
          <button
            ...
            className={[
              'flex items-center gap-1 py-1 px-2.5 rounded-xl transition-all',
        ...
        <button
          type="button"
          onClick={handleShare}
          className="flex items-center gap-1 text-slate-400 hover:text-slate-700 py-1 px-2.5 rounded-xl hover:bg-slate-100 transition-colors"
        >
